...
export function Contact() {
  ...
  const handleEmailClick = () => { const email = decodeEmail(encodedEmail); if (email) window.location.href = `mailto:${email}`; };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrors({});
    if (honeypot) { toast({ title: "Message Sent!" }); setFormData({ name: "", email: "", message: "" }); return; }
    if (formSubmittedAt && Date.now() - formSubmittedAt < 3000) { toast({ title: "Message Sent!" }); setFormData({ name: "", email: "", message: "" }); return; }
    const result = contactSchema.safeParse(formData);
    if (!result.success) {
      const fieldErrors: { name?: string; email?: string; message?: string } = {};
      result.error.errors.forEach((err) => { if (err.path[0]) fieldErrors[err.path[0] as keyof typeof fieldErrors] = err.message; });
      setErrors(fieldErrors);
      return;
    }
    setLoading(true);
    try {
      const { error } = await supabase.from("messages").insert([{ name: result.data.name, email: result.data.email, message: result.data.message }]);
      if (error) throw error;
      supabase.functions.invoke('send-contact-notification', { body: { name: result.data.name, email: result.data.email, message: result.data.message } });
              ...
              <div className="pt-4 border-t border-border/50">
                ...
                <div className="flex gap-2">
                  <a href={SOCIAL_LINKS.github} target="_blank" rel="noopener noreferrer">
                    <Button variant="outline" size="sm" className="rounded-full gap-2 text-xs border-border/50 hover:border-primary/30"><Github className="h-3.5 w-3.5" />GitHub</Button>
                  </a>
                  <a href={SOCIAL_LINKS.linkedin} target="_blank" rel="noopener noreferrer">
                    <Button variant="outline" size="sm" className="rounded-full gap-2 text-xs border-border/50 hover:border-primary/30"><Linkedin className="h-3.5 w-3.5" />LinkedIn</Button>
                  </a>
                </div>
              </div>
            </div>
